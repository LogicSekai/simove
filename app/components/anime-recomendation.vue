...
// const animeRecomendation = ref({
//     isLoading: true,
//     data: []
// })

// const fetchAnimeRecomendation = async () => {
//     animeRecomendation.value.isLoading = true
//     try {
//         const res = <any>await $fetch(config.uri + '/anime/recommendation/' + id) // Ganti dengan API kamu
//         animeRecomendation.value.data = res.data
//     } catch (error) {
//         console.error('Fetch failed:', error)
//     } finally {
//         animeRecomendation.value.isLoading = false
//     }
// }

// onMounted(() => {
//     fetchAnimeRecomendation()
// })
</script>
    ...
    <div>
        ...
        <div class="bg-card text-card-foreground flex flex-col gap-6 border shadow-sm py-0 px-4 rounded-md rounded-b-none md-4 mb-1">
            <div class="@container/card-header auto-rows-min grid-rows-[auto_auto] gap-1.5 has-data-[slot=card-action]:grid-cols-[1fr_auto] [.border-b]:pb-6 px-0 flex justify-between items-center">
                <div class="leading-none font-semibold py-2">Recommendation</div>
            </div>
        </div>
        <!-- Content -->
